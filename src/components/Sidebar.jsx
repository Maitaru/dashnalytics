    ...
    <nav className="min-h-screen relative shad z-40 w-[15rem] overflow-hidden">
      <div className="bg-white flex flex-col justify-betweeen gap-2 fixed left-0 top-0 min-h-screen w-[15rem] px-4 py-10">
        ...
        <div className="flex flex-col">
          {
            sidebarLinks.map(el => (
              <a key={el.id} href={el.link} className='flex gap-2 items-center rounded-md py-2 px-4 text-dark/80 bg-transparent hover:bg-dark/5 hover:text-main'><span className="text-lg md:text-xl w-8 flex-shirnk-0">{el.icon}</span>{el.title}</a>
            ))
          }
        </div>
      </div>
